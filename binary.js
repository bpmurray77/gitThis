...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const configurations = {
        ...
        'false_false_true': '<h2>AND 1</h2><p>The first AND gate takes inputs from A and B. With input B sending a 1, and input A sending a 0, this AND gate remains off since it only sends a 1 when both A AND B are on.</p><h2>XOR 1</h2><p>XOR gate 1 also takes inputs from A and B. With input B sending a 1, and input A sending a 0, XOR 1 outputs a 1. This signal is passed to both the second AND gate and the second XOR gate.</p><h2>AND 2</h2><p>AND 2 takes inputs from XOR 1 and the carry input. With XOR 1 sending this gate a 1, but the carry input off, this gate remains off since both inputs have to be on for this gate to pass a signal.</p><h2>XOR 2</h2><p>XOR 2 takes inputs from XOR 1 and the carry input. With XOR 1 sending a 1, and the carry input sending a 0, this gate sends a 1 to the sum output.</p><h2>OR</h2><p>The OR gate is connected to both the first and second AND gates. With both AND 1 and AND 2 sending a 0, the OR gate remains off.</p>',
        'false_true_false': 'Text for X: false, Y: true, Z: false',
        'false_true_true': 'Text for X: false, Y: true, Z: true',
        'true_false_false': '<h2>AND 1</h2><p>The first AND gate takes inputs from A and B. With input A sending a 1, and input B sending a 0, this AND gate remains off since it only sends a 1 when both A AND B are on.</p><h2>XOR 1</h2><p>XOR gate 1 also takes inputs from A and B. With input A sending a 1, and input B sending a 0, XOR 1 outputs a 1. This signal is passed to both the second AND gate and the second XOR gate.</p><h2>AND 2</h2><p>AND 2 takes inputs from XOR 1 and the carry input. With XOR 1 sending this gate a 1, but the carry input off, this gate remains off since both inputs have to be on for this gate to pass a signal.</p><h2>XOR 2</h2><p>XOR 2 takes inputs from XOR 1 and the carry input. With XOR 1 sending a 1, and the carry input sending a 0, this gate sends a 1 to the sum output.</p><h2>OR</h2><p>The OR gate is connected to both the first and second AND gates. With both AND 1 and AND 2 sending a 0, the OR gate remains off.</p>',
